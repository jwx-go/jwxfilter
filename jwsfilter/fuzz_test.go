...
// FuzzFilterRejectPartition checks that, for any parseable jws.Headers and
...
func FuzzFilterRejectPartition(f *testing.F) {
	...
	if err := headers.Set(jws.KeyIDKey, "kid-value"); err != nil {
		f.Fatal(err)
	}
	...
	f.Add(seed, "")
	f.Add(seed, jws.AlgorithmKey)
	f.Add(seed, jws.AlgorithmKey+","+jws.KeyIDKey)
	...
	f.Fuzz(func(t *testing.T, data []byte, names string) {
		...
		filter := jwsfilter.ByName(strings.Split(names, ",")...)
